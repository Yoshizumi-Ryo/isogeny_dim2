...
//mumfordから計算した方.
//こっちがあってる.
precomp_for_add_2:=AssociativeArray(); 
for zeta1 in set_zeta do
  zetas:=Ass_better_RP_zetas[zeta1];
  precomp_for_add_2[zetas]:=AssociativeArray();
  for etaa in set_zeta do
    assert (zeta1 eq zetas[1]);
    zeta2:=zetas[2];
    zeta3:=zetas[3];
...
precomp_for_ex_add:=AssociativeArray(); 
for zeta1 in set_zeta do
  zetas:=Ass_better_RP_zetas[zeta1];
  precomp_for_ex_add[zetas]:=AssociativeArray();
  for etaa in set_zeta do
    assert (zeta1 eq zetas[1]);
    zeta2:=zetas[2];
    zeta3:=zetas[3];
