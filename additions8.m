...
//different addition.
//楕円直積にも対応するように変更.
function add_2(alnp,alx,aly,alxmy)
  ".";
  u0:=u_of_ac(alnp);
  ux:=u_of_ac(alx);
  uy:=u_of_ac(aly);
      ...
      for zetas in RP_zetas do
        if (zetas[1] eq zeta1) then
          if (uxmy[zetas[2]] ne 0) then
            assert(u0[zetas[3]] ne 0);  
            assert(u0[zetas[4]] ne 0);
            uxpy[zeta1]:=sign_ellprod[zeta1]*calc_uxpy_3(u0,ux,uy,uxmy,zetas);
            break zetas;
          end if;
        end if;
      end for;
...
/*
//different addition.
function add_3(alnp,alx,aly,alxmy)
  ".";
  u0:=u_of_ac(alnp);
  ux:=u_of_ac(alx);
  uy:=u_of_ac(aly);
    ...
    for zetas in RP_zetas do
      if (zetas[1] eq zeta1) then
        if (uxmy[zetas[2]] ne 0) then
          assert(u0[zetas[3]] ne 0);  
          assert(u0[zetas[4]] ne 0);
          uxpy[zeta1]:=calc_uxpy_3(u0,ux,uy,uxmy,zetas);
          break zetas;
        end if;
      end if;
    end for;
